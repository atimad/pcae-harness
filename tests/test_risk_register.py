...
def _risk_data() -> dict:
    """Run risk-register once per worker; all field-checking tests share the result."""
...
def test_risk_register_exits_successfully(_risk_data) -> None:
    assert _risk_data is not None


def test_risk_register_output_is_valid_json(_risk_data) -> None:
    assert isinstance(_risk_data, dict)


def test_risk_register_envelope_fields(_risk_data) -> None:
    data = _risk_data
...
def test_risk_register_risks_is_list(_risk_data) -> None:
    assert isinstance(_risk_data["risks"], list)


def test_risk_register_risk_count_matches(_risk_data) -> None:
    assert _risk_data["risk_count"] == len(_risk_data["risks"])


def test_risk_register_required_fields(_risk_data) -> None:
    data = _risk_data
    required = [
        "risk_id",
        "risk_type",
...
def test_risk_register_risks_ordered_deterministically(_risk_data, _risk_data2) -> None:
    order1 = [(r["source_phase"], r["risk_type"], r["risk_id"]) for r in _risk_data["risks"]]
    order2 = [(r["source_phase"], r["risk_type"], r["risk_id"]) for r in _risk_data2["risks"]]
    assert order1 == order2


def test_risk_register_risk_ids_stable(_risk_data, _risk_data2) -> None:
    for r1, r2 in zip(_risk_data["risks"], _risk_data2["risks"]):
        assert r1["risk_id"] == r2["risk_id"]


def test_risk_register_has_required_initial_risks(_risk_data) -> None:
    types = {r["risk_type"] for r in _risk_data["risks"]}
...
def test_risk_register_accepted_risk_not_treated_as_mitigation(_risk_data) -> None:
    for risk in _risk_data["risks"]:
...
def test_risk_register_accepted_risk_safety_note(_risk_data) -> None:
    assert _risk_data["safety_notes"]["accepted_risk_is_not_mitigation"] is True


def test_risk_register_stale_signal_visible(_risk_data) -> None:
    stale = [r for r in _risk_data["risks"] if r["risk_status"] == "stale_signal"]
...
def test_risk_register_must_never_repeat_visible(_risk_data) -> None:
    mnr = [r for r in _risk_data["risks"] if r["risk_type"] == "must_never_repeat_risk"]
    assert len(mnr) > 0, "No must_never_repeat_risk found"
    raw_push = [r for r in _risk_data["risks"] if r["risk_type"] == "raw_push_exception_risk"]
    assert len(raw_push) > 0, "No raw_push_exception_risk found"
    hook_bypass = [r for r in _risk_data["risks"] if r["risk_type"] == "hook_bypass_exception_risk"]
    assert len(hook_bypass) > 0, "No hook_bypass_exception_risk found"


def test_risk_register_artifact_index_used(_risk_data) -> None:
    assert _risk_data["safety_notes"]["artifact_index_used"] is True


def test_risk_register_memory_snapshot_used(_risk_data) -> None:
    assert _risk_data["safety_notes"]["memory_snapshot_used"] is True


def test_risk_register_governance_timeline_used(_risk_data) -> None:
    assert _risk_data["safety_notes"]["governance_timeline_used"] is True


def test_risk_register_decision_log_used(_risk_data) -> None:
    assert _risk_data["safety_notes"]["decision_log_used"] is True


def test_risk_register_unknown_not_encoded_as_false(_risk_data) -> None:
    for risk in _risk_data["risks"]:
...
def test_risk_register_no_cache_files_created() -> None:
...
def test_risk_register_no_repository_files_created() -> None:
    result = subprocess.run(
...
def test_risk_register_no_authority_inference(_risk_data) -> None:
    notes = _risk_data["safety_notes"]
...
def test_risk_register_no_generated_cache(_risk_data) -> None:
    assert _risk_data["safety_notes"]["generated_cache_created"] is False
    assert _risk_data["safety_notes"]["pcae_storage_created"] is False


def test_risk_register_schema_version(_risk_data) -> None:
    assert _risk_data["schema_version"] == "0.1"


def test_risk_register_source_command(_risk_data) -> None:
    assert _risk_data["source_command"] == "pcae risk-register"
...
def test_decision_log_still_works() -> None:
    result = subprocess.run(
