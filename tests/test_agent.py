...
def test_46q_json_structure(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    for key in (
        "write_invocation_pilot", "pilot_lifecycle",
        "write_plan_model", "runtime_writable_contracts",
        "safety_gates", "pilot_result_model",
        "governance_boundaries", "advisory",
    ):
        assert key in data, f"missing top-level key: {key}"


def test_46q_design_fields(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    design = data["write_invocation_pilot"]
    for field in (
        "design_id", "phase", "title", "summary", "input_sources",
        "lifecycle_step_count", "write_plan_field_count",
        "runtime_contract_count", "safety_gate_count",
        "pilot_result_field_count", "execution_allowed",
        "human_review_required", "governance_boundaries", "future_evolution",
    ):
        assert field in design, f"missing design field: {field}"
    assert design["design_id"].startswith("cwip-")
    assert design["phase"] == "46Q"
    assert design["execution_allowed"] is False
    assert design["human_review_required"] is True
    assert design["lifecycle_step_count"] == 12
    assert design["write_plan_field_count"] == 13
    assert design["runtime_contract_count"] == 3
    assert design["safety_gate_count"] == 10
    assert design["pilot_result_field_count"] == 9


def test_46q_lifecycle_steps(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    lifecycle = data["pilot_lifecycle"]
    ...
    for expected in (
        "approved_prompt_artifact", "execution_authorization",
        "governed_write_candidate", "write_preflight",
        "human_write_approval", "runtime_writable_contract_check",
        "controlled_write_plan", "result_capture_plan",
        "rollback_plan", "audit_plan",
        "consensus_review_plan", "future_write_execution",
    ):
        assert expected in step_names, f"missing lifecycle step: {expected}"
    assert all(s["required"] is True for s in lifecycle)
    steps_ordered = [s["step"] for s in lifecycle]
    assert steps_ordered == list(range(1, 13))
    for step in lifecycle:
        for field in ("step", "name", "description", "required", "completed_by"):
            assert field in step, f"step missing field: {field}"


def test_46q_write_plan_model(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    wpm = data["write_plan_model"]
    assert wpm["model_name"] == "ControlledWritePlan"
    assert wpm["field_count"] == 13
    assert wpm["required_field_count"] == 13
    assert wpm["all_fields_immutable"] is True
    assert wpm["execution_allowed_always_false"] is True
    field_names = [f["name"] for f in wpm["fields"]]
    for expected in (
        "write_plan_id", "write_candidate_id", "selected_runtime",
        "selected_agent", "writable_contract", "file_scope",
        "allowed_operations", "forbidden_operations",
        "rollback_plan", "audit_plan",
        "consensus_required", "quality_review_required", "execution_allowed",
    ):
        assert expected in field_names, f"missing plan field: {expected}"
    exec_field = next(f for f in wpm["fields"] if f["name"] == "execution_allowed")
    assert "Always False" in exec_field["description"]


def test_46q_runtime_writable_contracts(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    rc = data["runtime_writable_contracts"]
    assert rc["contract_count"] == 3
    assert rc["all_contracts_available"] is True
    runtime_ids = rc["runtime_ids"]
    for expected in ("codex-local", "claude-local", "kimi-local"):
        assert expected in runtime_ids, f"missing runtime contract: {expected}"
    for contract in rc["contracts"]:
        for field in ("runtime", "writable_mode", "command_template",
                      "scope_enforcement", "contract_available"):
            assert field in contract, f"contract missing field: {field}"
        assert contract["contract_available"] is True
    codex = next(c for c in rc["contracts"] if c["runtime"] == "codex-local")
    assert "workspace-write" in codex["command_template"]
    claude = next(c for c in rc["contracts"] if c["runtime"] == "claude-local")
    assert "acceptEdits" in claude["command_template"]


def test_46q_safety_gates(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    sg = data["safety_gates"]
    assert sg["gate_count"] == 10
    assert sg["all_gates_blocking"] is True
    assert sg["human_approval_gate_always_required"] is True
    gate_names = [g["gate"] for g in sg["gates"]]
    for expected in (
        "prompt_approved", "authorization_valid", "write_candidate_valid",
        "file_scope_valid", "rollback_plan_present", "audit_plan_present",
        "runtime_writable_contract_available", "human_write_approval_present",
        "consensus_path_available", "quality_review_available",
    ):
        assert expected in gate_names, f"missing gate: {expected}"
    gate_ids = sg["gate_ids"]
    for expected in (f"cwip-g{i:02d}" for i in range(1, 11)):
        assert expected in gate_ids, f"missing gate id: {expected}"
...
def test_46q_pilot_result_model(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    rm = data["pilot_result_model"]
    assert rm["model_name"] == "ControlledWritePilotResult"
    ...
    field_names = [f["name"] for f in rm["fields"]]
    for expected in (
        "pilot_id", "selected_runtime", "selected_agent",
        "write_plan", "safety_gate_results", "blockers",
        "warnings", "write_execution_allowed", "human_review_required",
    ):
        assert expected in field_names, f"missing result field: {expected}"
    exec_field = next(f for f in rm["fields"] if f["name"] == "write_execution_allowed")
    assert "Always False" in exec_field["description"]
    hr_field = next(f for f in rm["fields"] if f["name"] == "human_review_required")
    assert "Always True" in hr_field["description"]


def test_46q_governance_boundaries(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    gb = data["governance_boundaries"]
    assert "pilot_may" in gb
...
def test_46q_execution_always_blocked(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    design = data["write_invocation_pilot"]
    assert design["execution_allowed"] is False
...
def test_46q_future_evolution(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    phases = [e["phase"] for e in data["write_invocation_pilot"]["future_evolution"]]
    assert "46R" in phases
    assert "46S" in phases
...
def test_46q_advisory(capsys) -> None:
    main(["write-invocation-pilot", "--json"])
    data = json.loads(capsys.readouterr().out)
    advisory = data["advisory"].lower()
    assert "simulation" in advisory
...
def test_46q_human_output_shows_all_sections(capsys) -> None:
    main(["write-invocation-pilot"])
    output = capsys.readouterr().out
    assert "Controlled write invocation pilot" in output
    assert "Pilot lifecycle" in output
    assert "ControlledWritePlan model" in output
    assert "Runtime writable contracts" in output
    assert "Write safety gates" in output
    assert "Pilot result model" in output
    assert "Governance boundaries" in output
    assert "simulation" in output.lower()
...
def test_47a_json_structure(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    for key in (
        "live_readonly_readiness", "readiness_areas", "runtime_results",
        "readiness_result_model", "blockers", "warnings",
        "recommendations", "governance_boundaries", "advisory",
    ):
        assert key in data, f"missing top-level key: {key}"


def test_47a_readiness_fields(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    ...
    for field in (
        ...
        "live_execution_recommended", "human_review_required",
        "area_count", "not_ready_area_count", "partially_ready_area_count",
        "ready_area_count", "active_blocker_count", "runtime_count",
        "warning_count", "readiness_statuses", "input_sources",
        "execution_allowed", "governance_boundaries", "future_evolution",
    ):
        assert field in r, f"missing readiness field: {field}"
    assert r["readiness_id"].startswith("lrora-")
    assert r["phase"] == "47A"
    assert r["area_count"] == 8
    assert r["execution_allowed"] is False


def test_47a_overall_status(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    r = data["live_readonly_readiness"]
    assert r["overall_status"] == "not_ready"
    assert r["not_ready_area_count"] >= 1
    assert (
        r["not_ready_area_count"] + r["partially_ready_area_count"] + r["ready_area_count"]
        == 8
    )


def test_47a_live_execution_not_recommended(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    assert data["live_readonly_readiness"]["live_execution_recommended"] is False


def test_47a_human_review_required(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    assert data["live_readonly_readiness"]["human_review_required"] is True
    assert data["governance_boundaries"]["human_review_required"] is True


def test_47a_readiness_areas(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    areas = data["readiness_areas"]
    assert len(areas) == 8
    area_names = [a["area"] for a in areas]
    for expected in (
        "authorization_readiness", "audit_readiness", "consensus_readiness",
        "runtime_readiness", "output_capture_readiness", "result_review_readiness",
        "quality_review_readiness", "human_approval_readiness",
    ):
        assert expected in area_names, f"missing readiness area: {expected}"
    for area in areas:
        for field in ("area", "description", "status", "rationale", "critical", "governance_source"):
        ...
        assert area["status"] in ("ready", "partially_ready", "not_ready")
    assert all(a["critical"] for a in areas)
    human_approval = next(a for a in areas if a["area"] == "human_approval_readiness")
    assert human_approval["status"] == "not_ready"


def test_47a_runtime_assessment(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    rt = data["runtime_results"]
    ...
    for runtime in rt["runtimes"]:
        for field in (
            "runtime", "status", "rationale", "adapter_type",
            "capabilities_verified", "live_execution_tested",
        ):
            assert field in runtime, f"runtime missing field: {field}"
        assert runtime["status"] in ("ready", "partially_ready", "not_ready")
        assert runtime["live_execution_tested"] is False
    kimi = next(r for r in rt["runtimes"] if r["runtime"] == "kimi-local")
    assert kimi["status"] == "not_ready"


def test_47a_readiness_result_model(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    m = data["readiness_result_model"]
    assert m["model_name"] == "LiveReadOnlyReadinessResult"
    ...
    field_names = [f["name"] for f in m["fields"]]
    for expected in (
        "readiness_id", "overall_status", "live_execution_recommended",
        "human_review_required", "runtime_results", "readiness_areas",
        "blockers", "warnings", "recommendations",
    ):
        assert expected in field_names, f"missing model field: {expected}"
    identity_fields = [f for f in m["fields"] if f["group"] == "identity"]
...
def test_47a_blockers(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    bl = data["blockers"]
    assert bl["blocker_count"] == 6
    assert bl["active_blocker_count"] == 6
    blocker_names = [b["blocker"] for b in bl["blockers"]]
    for expected in (
        "missing_authorization", "missing_audit_path", "missing_consensus_path",
        "missing_output_capture", "missing_runtime_support", "missing_review_workflow",
    ):
        assert expected in blocker_names, f"missing blocker: {expected}"
    for b in bl["blockers"]:
        for field in ("blocker", "description", "active", "severity"):
...
def test_47a_warnings(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    warnings = data["warnings"]
    ...
    assert "quality review" in combined


def test_47a_recommendations(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    rec = data["recommendations"]
    assert "readiness_recommendation" in rec
    assert "required_follow_up_phases" in rec
    assert "execution_authorization_recommendation" in rec
    assert "47C" in rec["required_follow_up_phases"]


def test_47a_governance_boundaries(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    gb = data["governance_boundaries"]
    assert gb["execution_allowed"] is False
...
def test_47a_advisory(capsys) -> None:
    main(["live-readonly-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    advisory = data["advisory"].lower()
    assert "informational" in advisory
...
def test_47a_human_output_shows_all_sections(capsys) -> None:
    main(["live-readonly-readiness"])
    output = capsys.readouterr().out
    assert "Live read-only execution readiness assessment" in output
    assert "Assessment results" in output
    assert "Readiness areas" in output
    assert "Runtime assessment" in output
    assert "LiveReadOnlyReadinessResult model" in output
    assert "Blockers" in output
    assert "Warnings" in output
    assert "Recommendations" in output
    assert "Governance boundaries" in output
...
def test_47b_json_structure(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    for key in (
        "live_write_readiness", "readiness_areas", "runtime_results",
        "readiness_result_model", "blockers", "risks",
        "recommendations", "governance_boundaries", "advisory",
    ):
        assert key in data, f"missing top-level key: {key}"
...
def test_47b_readiness_fields(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    r = data["live_write_readiness"]
    for field in (
        "readiness_id", "phase", "title", "summary", "overall_status",
        "live_write_recommended", "human_review_required",
        "area_count", "not_ready_area_count", "partially_ready_area_count",
        "ready_area_count", "active_blocker_count", "risk_count", "runtime_count",
        "readiness_statuses", "input_sources", "execution_allowed",
        "file_modification_allowed", "governance_boundaries", "future_evolution",
    ):
        assert field in r, f"missing readiness field: {field}"
    assert r["readiness_id"].startswith("lwra-")
    assert r["phase"] == "47B"
    assert r["area_count"] == 9
    assert r["execution_allowed"] is False
    assert r["file_modification_allowed"] is False


def test_47b_overall_status(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    r = data["live_write_readiness"]
    assert r["overall_status"] == "not_ready"
...
def test_47b_live_write_not_recommended(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    assert data["live_write_readiness"]["live_write_recommended"] is False


def test_47b_human_review_required(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    assert data["live_write_readiness"]["human_review_required"] is True
    assert data["governance_boundaries"]["human_review_required"] is True


def test_47b_readiness_areas(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    areas = data["readiness_areas"]
    assert len(areas) == 9
    area_names = [a["area"] for a in areas]
    for expected in (
        "write_authorization_readiness", "file_scope_readiness", "rollback_readiness",
        "audit_readiness", "consensus_readiness", "quality_review_readiness",
        "result_review_readiness", "runtime_writable_readiness",
        "human_write_approval_readiness",
    ):
        assert expected in area_names, f"missing readiness area: {expected}"
    for area in areas:
        for field in ("area", "description", "status", "rationale", "critical", "governance_source"):
        ...
        assert area["status"] in ("ready", "partially_ready", "not_ready")
    assert all(a["critical"] for a in areas)
    human_approval = next(a for a in areas if a["area"] == "human_write_approval_readiness")
    assert human_approval["status"] == "not_ready"


def test_47b_runtime_writable_assessment(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    rt = data["runtime_results"]
    assert rt["runtime_count"] == 3
    ...
    for runtime in rt["runtimes"]:
        for field in (
            "runtime", "status", "rationale", "adapter_type",
            "writable_contract", "contract_verified", "live_write_tested",
        ):
            assert field in runtime, f"runtime missing field: {field}"
        assert runtime["status"] in ("ready", "partially_ready", "not_ready")
        assert runtime["live_write_tested"] is False
        assert runtime["contract_verified"] is False
    kimi = next(r for r in rt["runtimes"] if r["runtime"] == "kimi-local")
    assert kimi["status"] == "not_ready"
    codex = next(r for r in rt["runtimes"] if r["runtime"] == "codex-local")
    assert codex["writable_contract"] == "workspace-write"
    claude = next(r for r in rt["runtimes"] if r["runtime"] == "claude-local")
    assert claude["writable_contract"] == "acceptEdits"


def test_47b_readiness_result_model(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    m = data["readiness_result_model"]
    assert m["model_name"] == "LiveWriteReadinessResult"
    assert m["field_count"] == 9
    assert m["required_field_count"] == 9
    assert m["immutable_field_count"] == 2
    assert set(m["groups"]) == {"identity", "result", "findings"}
    field_names = [f["name"] for f in m["fields"]]
    for expected in (
        "readiness_id", "overall_status", "live_write_recommended",
        "human_review_required", "runtime_results", "readiness_areas",
        "blockers", "warnings", "recommendations",
    ):
        assert expected in field_names, f"missing model field: {expected}"
...
def test_47b_blockers(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    bl = data["blockers"]
    assert bl["blocker_count"] == 8
    ...
    blocker_names = [b["blocker"] for b in bl["blockers"]]
    for expected in (
        "missing_write_authorization", "missing_rollback_readiness",
        "missing_runtime_writable_support", "missing_audit_path",
        "missing_consensus_path", "missing_quality_review",
        "missing_result_review", "unresolved_file_scope_governance",
    ):
        assert expected in blocker_names, f"missing blocker: {expected}"
    for b in bl["blockers"]:
        for field in ("blocker", "description", "active", "severity"):
...
def test_47b_risks(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    ri = data["risks"]
    assert ri["risk_count"] == 6
...
def test_47b_recommendations(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    rec = data["recommendations"]
    assert "readiness_recommendation" in rec
    ...
    assert "execution_authorization_recommendation" in rec
    phases = rec["required_follow_up_phases"]
    for expected in ("47C", "47D", "47E"):
        assert expected in phases, f"missing follow-up phase: {expected}"


def test_47b_governance_boundaries(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    gb = data["governance_boundaries"]
    assert gb["execution_allowed"] is False
    assert gb["file_modification_allowed"] is False
    assert gb["human_review_required"] is True
    assert gb["read_only"] is True
    may = " ".join(gb["workflow_may"]).lower()
    for allowed in ("assess readiness", "identify blockers", "generate recommendations"):
...
def test_47b_advisory(capsys) -> None:
    main(["live-write-readiness", "--json"])
    data = json.loads(capsys.readouterr().out)
    advisory = data["advisory"].lower()
    assert "informational" in advisory
...
def test_47b_human_output_shows_all_sections(capsys) -> None:
    main(["live-write-readiness"])
    output = capsys.readouterr().out
    assert "Live write execution readiness assessment" in output
    assert "Assessment results" in output
    assert "Readiness areas" in output
    assert "Runtime writable assessment" in output
    assert "LiveWriteReadinessResult model" in output
    assert "Blockers" in output
    assert "Risks" in output
    assert "Recommendations" in output
    assert "Governance boundaries" in output
