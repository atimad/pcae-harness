...
def test_scope_gate_has_scope_evaluation():
    data = _run_gate(["--requested-action", "source_mutation",
...
def test_backend_gate_has_backend_evaluation():
    data = _run_gate(["--requested-action", "backend_invocation",
...
def test_adoption_gate_has_adoption_evaluation():
    data = _run_gate(["--requested-action", "adoption",
...
def test_test_mutation_gate_has_mutation_evaluation():
    data = _run_gate(["--requested-action", "test_mutation",
...
def test_all_gates_authorization_false_with_all_flags():
    data = _run_gate(["--requested-action", "adoption",
                      ...
                      "--requested-backend", "claude", "--prompt-present",
                      "--commit-message-present", "--push-target", "origin/main"])
    for g in data["gates"]:
        assert g["authorization_granted"] is False, f"{g['gate_id']}"
        assert g["enforcement_performed"] is False, f"{g['gate_id']}"
...
def test_no_cache_or_state_created():
    ...
    _run_gate(["--requested-action", "backend_invocation",
               "--requested-backend", "claude", "--prompt-present"])
    _run_gate(["--requested-action", "commit", "--commit-message-present"])
    _run_gate(["--requested-action", "push", "--push-target", "origin/main"])
...
def test_no_repository_mutation():
    ...
    before = r1.stdout
    _run_gate(["--requested-action", "source_mutation",
               "--requested-file", "src/example.py", "--human-approved"])
    _run_gate(["--requested-action", "push", "--push-target", "origin/main"])
