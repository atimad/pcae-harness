...
def context(item: ra.RuntimeInvocationApproval, **changes: object) -> ra.InvocationRequestContext:
    ...
    adapter_changes = {
        ...
        if key in {"adapter_id", "descriptor_version", "descriptor_digest", "target_config_digest"}
    }
    if adapter_changes:
        values["adapter_binding"] = dataclasses.replace(item.adapter_binding, **adapter_changes)
    if "requested_capability" in changes:
        values["approval_scope"] = dataclasses.replace(
            item.approval_scope,
            requested_capability=changes.pop("requested_capability"),
        )
    values.update(changes)
...
def inputs(item: ra.RuntimeInvocationApproval, **changes: object) -> rdp.RuntimeDispatchRequestConstructionInput:
    values: dict[str, object] = {
        "repository_identity": item.subject.repository_identity,
        "base_commit": item.freshness_snapshot.head_commit,
        "task_id": item.subject.task_id,
        "task_contract_digest": item.freshness_snapshot.task_contract_digest,
        ...
        "process_profile_ref": pbf.RuntimeDispatchFilesystemScopeRef(
            item.approval_scope.process_profile_ref.artifact_id,
            item.approval_scope.process_profile_ref.artifact_digest,
        ),
        "effect_class": item.approval_scope.effect_class,
        "network_requirement": item.approval_scope.network_required,
        "resource_budget": pbf.RuntimeDispatchFilesystemScopeRef("budget", "4" * 64),
    }
    values.update(changes)
...
def new_identity(
    bound_inputs: rdp.RuntimeDispatchRequestConstructionInput,
    *,
...
def build_request(item: ra.RuntimeInvocationApproval, *, simulation_only: bool):
    ...
    identity = new_identity(bound_inputs, invocation_id=item.subject.invocation_id)
...
def test_incomplete_schema_type_enforcement_is_closed(path, value):
    raw = approval().to_dict()
    cursor = raw
    for key in path[:-1]:
        cursor = cursor[key]
...
def test_preview_digest_tamper_fails_closed():
    item = approval()
...
def test_descriptor_version_is_bound():
    item = approval()
...
def test_duplicate_json_identity_fails_closed(tmp_path: Path):
    store = RuntimeInvocationApprovalStore(tmp_path)
    item = approval()
...
def test_approval_directory_symlink_fails_closed(tmp_path: Path):
    store = RuntimeInvocationApprovalStore(tmp_path)
    item = approval()
    ...
    with pytest.raises(ApprovalStoreIntegrityError):
        store.create(item)
    assert not (external / "approval.json").exists()


def test_precreated_tmp_symlink_cannot_overwrite_external_file(tmp_path: Path):
    store = RuntimeInvocationApprovalStore(tmp_path)
    item = approval()
...
def test_precreated_tmp_hardlink_cannot_overwrite_external_file(tmp_path: Path):
    store = RuntimeInvocationApprovalStore(tmp_path)
    item = approval()
    approval_dir = tmp_path / STORE_ROOT / item.approval_id
    approval_dir.mkdir(parents=True)
...
def test_publish_failure_leaves_no_final_artifact(tmp_path: Path, monkeypatch):
    store = RuntimeInvocationApprovalStore(tmp_path)
    item = approval()
    original = os.link

    def fail_link(src, dst, *args, **kwargs):
        if src == "approval.json.tmp":
...
def test_creation_rejects_chronologically_earlier_expiry():
...
def test_attempt_id_format_uniqueness_and_identity_distinction():
    item = approval()
    bound_inputs = inputs(item)
    first = new_identity(bound_inputs)
    second = new_identity(bound_inputs)
...
def test_idempotency_is_deterministic_across_processes():
    ...
    projection = rdp.canonical_runtime_dispatch_projection(
        inputs(item), invocation_id=item.subject.invocation_id
    )
...
def test_idempotency_changes_for_represented_identity_fields(field: str, value: str):
    ...
    assert rdp.compute_runtime_dispatch_idempotency_key(
        rdp.canonical_runtime_dispatch_projection(first, invocation_id=item.subject.invocation_id)
    ) != rdp.compute_runtime_dispatch_idempotency_key(
        rdp.canonical_runtime_dispatch_projection(second, invocation_id=item.subject.invocation_id)
    )


def test_idempotency_projection_includes_all_identity_critical_contract_fields():
    ...
    assert {
        ...
        "effect_class", "network_requirement", "resource_budget",
    } <= field_names
    projected = rdp.canonical_runtime_dispatch_projection(
        inputs(approval()), invocation_id="inv-" + "9" * 32
    )
    assert {
        "invocation_id", "base_commit", "task_contract_digest",
        "process_profile_ref", "effect_class", "network_requirement",
        "resource_budget",
    } <= projected.keys()


def test_distinct_invocations_never_share_key_by_construction():
    item = approval()
    bound_inputs = inputs(item)
    first = new_identity(bound_inputs)
    second = new_identity(bound_inputs)
...
def test_runtime_context_is_mandatory_for_runtime_dispatch():
    ...
    request = pbf.PermissionBrokerRequest(
        request_id="pbr-forged", timestamp=NOW,
        action_type=pbf.ACTION_TYPE_RUNTIME_DISPATCH,
        execution_class=pbf.EXECUTION_CLASS_ADAPTER,
        task_id="task-3w1", phase_id="149O.20L.7O.3W.1",
        requested_component="COMP-006", requested_capability="bounded.execute",
        requested_resource=None, evidence_available=True, approval_present=True,
        simulation_only=True, runtime_dispatch_context=None,
    )
    decision = pbf.PermissionBroker().evaluate(request)
    assert decision.decision == pbf.DECISION_DENY
...
def test_public_projection_forgery_is_rejected():
    item = approval()
    ...
    bound_inputs = inputs(item)
    identity = new_identity(bound_inputs)
