...
Independent Real-Host Verification.

This phase's live findings (fresh SSH to `hac-dell`, independent stat/
getfacl/HMIC/HBDC re-derivation) are not reproducible in CI -- no route
to `hac-dell` exists here. These tests instead assert the local
invariants that must hold around an independent-verification-only
phase: the governing CHGR still verifies unchanged and remains the
sole record naming this exact target/transition, no new CHGR was
published, no RepositoryIdentity or DeploymentBinding artifact exists
locally, no production source path changed since the election-time
SHA, and this phase's own doc records the exact live values reported
above (self-consistency, not re-execution).
"""
...
_DOC_PATH = (
    _REPO_ROOT
    / "docs"
    / "PHASE_149O_20L_7O_2A_5_REPOSITORYIDENTITY_WRITE_PATH_REMEDIATION_INDEPENDENT_REAL_HOST_VERIFICATION.md"
)
...
# ═══════════════════════════════════════════════════════════════════════════
# 2. Zero local mutation artifacts -- this is a verification-only phase
#    and must leave zero RepositoryIdentity/DeploymentBinding trace.
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# 4. Doc self-consistency: the exact live values this phase's
#    verification claims must be present, verbatim, in the phase doc.
# ═══════════════════════════════════════════════════════════════════════════
...
class TestDocRecordsExactExpectedValues:
    ...
    def test_doc_records_fresh_ssh_session_not_reused(self) -> None:
        text = _doc_text()
        assert "fresh SSH session" in text
        assert "did not reuse the 7O.2A.4 session" in text

    def test_doc_records_exact_mode_and_owner(self) -> None:
        text = _doc_text()
        assert "1770" in text
        assert "root:pcae" in text

    def test_doc_records_no_extended_acl(self) -> None:
        text = _doc_text()
        assert "user::rwx" in text
        assert "group::rwx" in text
        assert "No extended ACL" in text or "no extended ACL" in text

    def test_doc_records_unchanged_source_sha(self) -> None:
        assert _ELECTION_TIME_SHA in _doc_text()

    def test_doc_records_hmic_digest_live_invocation(self) -> None:
        text = _doc_text()
        assert _EXPECTED_HMIC_DIGEST in text
    ...
    def test_doc_records_hbdc_non_compliant_sole_residual_two_runs(self) -> None:
        text = _doc_text()
    ...
    def test_doc_records_mutation_inventory_exactly_one_chmod(self) -> None:
        text = _doc_text()
    ...
    def test_doc_final_verdict_is_independently_verified(self) -> None:
        text = _doc_text()
        assert "INDEPENDENTLY VERIFIED -- PERMISSION REMEDIATION COMPLETE" in text

    def test_doc_recommends_exact_next_phase(self) -> None:
        assert "149O.20L.7O.2B" in _doc_text()

    def test_doc_does_not_claim_repositoryidentity_creation(self) -> None:
        text = _doc_text()
        assert "No RepositoryIdentity was created" in text
