...
def test_project_state_exits_successfully(_project_state) -> None:
    assert _project_state is not None


def test_project_state_output_is_valid_json(_project_state) -> None:
    assert isinstance(_project_state, dict)


def test_project_state_envelope_fields(_project_state) -> None:
    data = _project_state
...
def test_project_state_snapshot_is_dict(_project_state) -> None:
    assert isinstance(_project_state["snapshot"], dict)


def test_project_state_required_fields(_project_state) -> None:
    snap = _project_state["snapshot"]
    required = [
        "snapshot_id",
        "snapshot_version",
        "snapshot_status",
...
def test_project_state_latest_completed_phase(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert snap["latest_completed_phase"] is not None
    assert snap["latest_completed_phase"] != "unknown"


def test_project_state_recommended_next_phase(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert snap["recommended_next_phase"] is not None


def test_project_state_active_risks_populated(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert isinstance(snap["active_risks"], list)
    assert len(snap["active_risks"]) > 0


def test_project_state_accepted_risks_separate(_project_state) -> None:
    snap = _project_state["snapshot"]
...
def test_project_state_stale_signals_visible(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert isinstance(snap["stale_signals"], list)
    assert len(snap["stale_signals"]) > 0


def test_project_state_must_never_repeat_visible(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert isinstance(snap["must_never_repeat_controls"], list)
    assert len(snap["must_never_repeat_controls"]) > 0


def test_project_state_next_safe_actions_present(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert isinstance(snap["next_safe_actions"], list)
    assert len(snap["next_safe_actions"]) > 0


def test_project_state_next_safe_actions_are_recommendations(_project_state) -> None:
    snap = _project_state["snapshot"]
    data = _project_state
...
def test_project_state_forbidden_actions_present(_project_state) -> None:
    snap = _project_state["snapshot"]
    assert isinstance(snap["forbidden_actions"], list)
    assert len(snap["forbidden_actions"]) > 0


def test_project_state_authorization_booleans_explicit(_project_state) -> None:
    snap = _project_state["snapshot"]
    auth_fields = [
        "execution_authorized",
        "backend_invocation_authorized",
...
def test_project_state_high_risk_auth_false(_project_state) -> None:
    snap = _project_state["snapshot"]
...
def test_project_state_artifact_index_used(_project_state) -> None:
    assert _project_state["safety_notes"]["artifact_index_used"] is True


def test_project_state_memory_snapshot_used(_project_state) -> None:
    assert _project_state["safety_notes"]["memory_snapshot_used"] is True


def test_project_state_governance_timeline_used(_project_state) -> None:
    assert _project_state["safety_notes"]["governance_timeline_used"] is True


def test_project_state_decision_log_used(_project_state) -> None:
    assert _project_state["safety_notes"]["decision_log_used"] is True


def test_project_state_risk_register_used(_project_state) -> None:
    assert _project_state["safety_notes"]["risk_register_used"] is True


def test_project_state_unknown_not_encoded_as_false(_project_state) -> None:
    snap = _project_state["snapshot"]
...
def test_project_state_no_cache_files_created() -> None:
...
def test_project_state_no_repository_files_created() -> None:
    result = subprocess.run(
...
def test_project_state_no_authority_inference(_project_state) -> None:
    notes = _project_state["safety_notes"]
...
def test_project_state_no_generated_cache(_project_state) -> None:
    assert _project_state["safety_notes"]["generated_cache_created"] is False
    assert _project_state["safety_notes"]["pcae_storage_created"] is False
...
def test_project_state_source_command(_project_state) -> None:
    assert _project_state["source_command"] == "pcae project-state"


def test_project_state_layer_summary(_project_state) -> None:
    assert "layer_summary" in _project_state
    layers = _project_state["layer_summary"]
...
def test_risk_register_still_works() -> None:
    result = subprocess.run(
