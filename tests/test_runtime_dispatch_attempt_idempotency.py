...
FIXED_INVOCATION_ID = "inv-" + "0" * 32


def canonical(inputs):
    return rdp.canonical_runtime_dispatch_projection(
        inputs, invocation_id=FIXED_INVOCATION_ID
    )
...
def test_same_invocation_retry_shares_idempotency_key_but_not_attempt_id(tmp_path: Path):
    ...
    inputs = dispatch_inputs()
    id1 = new_dispatch_identity(inputs, root=tmp_path)
    id2 = new_dispatch_identity(inputs, root=tmp_path, invocation_id=id1.invocation_id)
...
def test_idempotency_key_is_pure_function_impossible_to_hold_constant_across_change():
    ...
    proj_a = canonical(inputs_a)
    proj_b = canonical(inputs_b)
...
def test_same_attempt_id_different_content_hard_collision(tmp_path: Path):
    tracker = rdp.RuntimeDispatchIdentityTracker(tmp_path)
    inputs_a = dispatch_inputs(task_id="task-a")
    inputs_b = dispatch_inputs(task_id="task-b")
    identity_a = rdp.new_runtime_dispatch_identity(inputs_a, identity_tracker=tracker)
    ...
    identity_b_forced = dataclasses.replace(
        rdp.new_runtime_dispatch_identity(inputs_b, identity_tracker=tracker),
        attempt_id=identity_a.attempt_id,
    )
    with pytest.raises(rdp.RuntimeDispatchConstructionError):
        tracker.register(identity_b_forced)


def test_distinct_invocation_id_changes_idempotency_key(tmp_path: Path):
    ...
    inputs = dispatch_inputs()
    identity_1 = new_dispatch_identity(inputs, root=tmp_path)
    identity_2 = new_dispatch_identity(inputs, root=tmp_path)
    assert identity_1.invocation_id != identity_2.invocation_id
    assert identity_1.idempotency_key != identity_2.idempotency_key


def test_same_attempt_id_identical_content_is_a_collision(tmp_path: Path):
    """An attempt ID names exactly one concrete registration."""
    tracker = rdp.RuntimeDispatchIdentityTracker(tmp_path)
    inputs = dispatch_inputs()
    identity = rdp.new_runtime_dispatch_identity(inputs, identity_tracker=tracker)
    with pytest.raises(rdp.RuntimeDispatchConstructionError):
...
def test_pb_decision_is_freshly_evaluated_per_request_never_cached_across_attempt_change(tmp_path: Path):
    ...
    inputs = dispatch_inputs()
    identity_1 = new_dispatch_identity(inputs, root=tmp_path)
    identity_2 = new_dispatch_identity(inputs, root=tmp_path, invocation_id=identity_1.invocation_id)
