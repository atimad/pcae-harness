...
def test_public_api_matches_independently_derived_inventory():
    expected_public_names = {
        ...
        "OperatorReview",
        "RecoveryAction",
        # Narrowed by Phase 136AL (Typed Model Implementation Group 7):
        # `NotificationAuthorityBinding` and its record-local value types
        # are now legitimate, authorized public exports.
        "NotificationAuthorityBinding",
        "DeliveryState",
        ...
        "CompatibilityState",
        "CompatibilityRole",
        # Narrowed by Phase 136AT (Typed Model Implementation Group 11):
        # `QuarantineRecord` and its record-local value types are now
        # legitimate, authorized public exports -- the sixteenth and
        # final Stage 3 record-family model.
        "QuarantineRecord",
        "ObjectType",
        "QuarantineState",
    }
...
def test_no_record_family_model_class_exists_anywhere_in_package():
    ...
    still_forbidden = FORBIDDEN_MODEL_CLASS_NAMES - authorized_groups_2_through_11
...
def test_authority_package_files_present_on_disk_for_packaging():
    ...
    # (Group 10, `CompatibilityState`) is now authorized too. Narrowed
    # further by Phase 136AT: the same module now also carries
    # `QuarantineRecord` (Group 11) -- every module name in this set is
    # now fully authorized; no later-group module name remains.
    expected_modules = {
        "__init__.py",
        "cas_expectation.py",
        "digest.py",
