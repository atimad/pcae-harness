...
from pcae.commands.phase import (
    ...
    run_phase_runner_execution_trace_approval_show,
    run_phase_runner_execution_trace_approve,
    ...
    run_phase_backend_created_output_adoption_execution,
    ...
    run_phase_backend_created_output_adoption_push_approval,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    preflight_parser = subparsers.add_parser(
        "preflight",
        help="Scope gate preflight evaluation (non-authorizing).",
    )
    preflight_subparsers = preflight_parser.add_subparsers(
        dest="preflight_command", required=True,
    )
    preflight_scope_parser = preflight_subparsers.add_parser(
        "scope",
        help="Evaluate requested action and files against active task contract scope.",
    )
    preflight_scope_parser.add_argument(
        "--json",
        action="store_true",
        help="Print machine-readable JSON scope preflight output.",
    )
    preflight_scope_parser.add_argument(
        "--requested-action",
        metavar="ACTION",
        required=True,
        help="Action to evaluate (e.g., read, source_mutation, commit).",
    )
    preflight_scope_parser.add_argument(
        "--requested-file",
        action="append",
    ...
    phase_queue_hygiene_parser = phase_queue_subparsers.add_parser(
        "hygiene",
        help="Detect and optionally clear placeholder/test queue entries.",
    )
    phase_queue_hygiene_parser.add_argument(
        "--json",
        action="store_true",
        help="Print machine-readable JSON output.",
    )
    phase_queue_hygiene_parser.add_argument(
        "--clear-placeholders",
        action="store_true",
        help="Clear detected placeholder entries from the queue.",
    )
    phase_queue_hygiene_parser.add_argument(
        "--confirm",
        action="store_true",
    ...
    phase_prompt_show_parser = phase_subparsers.add_parser(
        "prompt-show",
        help="Display the latest captured phase prompt.",
    )
    phase_prompt_show_parser.add_argument(
        "--json",
        action="store_true",
        help="Print machine-readable JSON output with metadata and content.",
    )
    phase_prompt_show_parser.set_defaults(handler=run_phase_prompt_show)

    phase_prompt_list_parser = phase_subparsers.add_parser(
        "prompt-list",
        help="List captured phase prompts.",
    )
    phase_prompt_list_parser.add_argument(
        "--json",
        action="store_true",
        help="Print machine-readable JSON output.",
    )
    ...
    phase_runner_policy_parser = phase_subparsers.add_parser(
        "runner-policy",
        help="Display the stop-condition policy matrix for bounded phase runner behavior.",
    )
    phase_runner_policy_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    ...
    phase_runner_execution_preflight_parser = phase_subparsers.add_parser(
        ...
        help="Design-only preflight specification for future runner execution (no execution available).",
    )
    phase_runner_execution_preflight_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    ...
    phase_runner_execution_trace_approve_parser = phase_subparsers.add_parser(
        "runner-execution-trace-approve",
        help="Approve a runner no-op trace (does not authorize execution).",
    )
    phase_runner_execution_trace_approve_parser.add_argument(
    ...
    phase_runner_execution_trace_approve_parser.set_defaults(
        handler=run_phase_runner_execution_trace_approve
    )

    phase_runner_execution_trace_approval_show_parser = phase_subparsers.add_parser(
        "runner-execution-trace-approval-show",
        help="Show the latest persisted trace approval artifact.",
    )
    phase_runner_execution_trace_approval_show_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_runner_execution_trace_approval_show_parser.set_defaults(
        handler=run_phase_runner_execution_trace_approval_show
    )
    ...
    phase_single_runner_contract_parser = phase_subparsers.add_parser(
        "single-runner-contract",
        help="Show the design-only single-phase runner contract (does not enable execution).",
    )
    phase_single_runner_contract_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_single_runner_contract_parser.add_argument(
    ...
    phase_real_execution_disabled_proof_parser = phase_subparsers.add_parser(
        "real-execution-disabled-proof",
        help="Prove that real execution is still disabled across all artifacts.",
    )
    phase_real_execution_disabled_proof_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_real_execution_disabled_proof_parser.add_argument(
        "--save", action="store_true", help="Persist the proof artifact."
    )
    ...
    phase_single_runner_activate_parser.add_argument(
        "--execute", action="store_true", help="Execute activation (create task). Without this, dry-run only."
    )
    phase_single_runner_activate_parser.set_defaults(
        handler=run_phase_single_runner_activate
    )

    phase_single_runner_activation_show_parser = phase_subparsers.add_parser(
        "single-runner-activation-show",
        help="Show the latest persisted activation artifact.",
    )
    phase_single_runner_activation_show_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    ...
    phase_activated_task_prompt_capture_parser = phase_subparsers.add_parser(
        "activated-task-prompt-capture",
        help="Dry-run activated task prompt capture (Phase 74Y).",
    )
    phase_activated_task_prompt_capture_parser.add_argument(
    ...
    phase_activated_task_prompt_capture_parser.set_defaults(
        handler=run_phase_activated_task_prompt_capture
    )

    phase_activated_task_prompt_capture_smoke_parser = phase_subparsers.add_parser(
        "activated-task-prompt-capture-smoke",
        help="Run activated task output-only prompt capture smoke (Phase 74Z).",
    )
    phase_activated_task_prompt_capture_smoke_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_activated_task_prompt_capture_smoke_parser.add_argument(
        "--save", action="store_true", help="Persist the smoke artifact."
    ...
    phase_activated_task_prompt_capture_smoke_parser.add_argument(
        ...
        help="Allow real claude-deepseek invocation with the activated task prompt (opt-in).",
    )
    phase_activated_task_prompt_capture_smoke_parser.set_defaults(
        handler=run_phase_activated_task_prompt_capture_smoke
    )

    phase_activated_task_prompt_capture_show_parser = phase_subparsers.add_parser(
        "activated-task-prompt-capture-show",
        help="Show the latest activated task prompt capture artifact (Phase 74Z).",
    )
    phase_activated_task_prompt_capture_show_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    ...
    # Phase 75G
    phase_captured_output_manual_apply_approval_contract_parser = phase_subparsers.add_parser(
        "captured-output-manual-apply-approval-contract",
        help="Manual apply approval contract for captured output (Phase 75G).",
    )
    ...
    # Phase 75I.1
    phase_governance_bypass_classification_parser = phase_subparsers.add_parser(
        "governance-bypass-classification",
        help="Classify governance bypass findings (Phase 75I.1).",
    )
    ...
    # Phase 76C
    phase_captured_output_manual_apply_execution_preflight_parser = phase_subparsers.add_parser(
        "captured-output-manual-apply-execution-preflight",
        help="Manual apply execution preflight for captured output (Phase 76C).",
    )
    ...
    # Phase 76G
    phase_captured_output_manual_apply_final_summary_parser = phase_subparsers.add_parser(
        "captured-output-manual-apply-final-summary",
        help="Final lifecycle summary for captured output manual apply (Phase 76G).",
    )
    phase_captured_output_manual_apply_final_summary_parser.add_argument("--json", action="store_true")
    phase_captured_output_manual_apply_final_summary_parser.add_argument("--save", action="store_true")
    phase_captured_output_manual_apply_final_summary_parser.set_defaults(handler=run_phase_captured_output_manual_apply_final_summary)

    phase_captured_output_manual_apply_final_summary_show_parser = phase_subparsers.add_parser(
        "captured-output-manual-apply-final-summary-show",
        help="Show latest final lifecycle summary (Phase 76G).",
    )
    ...
    # Phase 77O
    phase_backend_created_output_adoption_approval_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-approval",
        help="Adoption approval for backend-created output (Phase 77O).",
    )
    phase_backend_created_output_adoption_approval_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_approval_parser.add_argument("--save", action="store_true")
    phase_backend_created_output_adoption_approval_parser.add_argument("--approve", action="store_true")
    phase_backend_created_output_adoption_approval_parser.add_argument("--approved-by", type=str, default="")
    phase_backend_created_output_adoption_approval_parser.add_argument("--reason", type=str, default="")
    phase_backend_created_output_adoption_approval_parser.set_defaults(handler=run_phase_backend_created_output_adoption_approval)

    phase_backend_created_output_adoption_approval_show_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-approval-show",
        help="Show latest backend-created output adoption approval artifact (Phase 77O).",
    )
    ...
    # Phase 77Q
    phase_backend_created_output_adoption_execution_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-execution",
        help="Adoption execution for backend-created output — stages gitignored file (Phase 77Q).",
    )
    phase_backend_created_output_adoption_execution_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_execution_parser.add_argument("--save", action="store_true")
    phase_backend_created_output_adoption_execution_parser.add_argument("--dry-run", action="store_true")
    phase_backend_created_output_adoption_execution_parser.add_argument("--execute", action="store_true")
    phase_backend_created_output_adoption_execution_parser.set_defaults(handler=run_phase_backend_created_output_adoption_execution)

    phase_backend_created_output_adoption_execution_show_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-execution-show",
        help="Show latest backend-created output adoption execution artifact (Phase 77Q).",
    )
    phase_backend_created_output_adoption_execution_show_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_execution_show_parser.set_defaults(handler=run_phase_backend_created_output_adoption_execution_show)

    # Phase 77R
    phase_backend_created_output_adoption_commit_approval_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-commit-approval",
        help="Commit approval for staged backend-created output (Phase 77R).",
    )
    phase_backend_created_output_adoption_commit_approval_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_commit_approval_parser.add_argument("--save", action="store_true")
    phase_backend_created_output_adoption_commit_approval_parser.add_argument("--approve", action="store_true")
    phase_backend_created_output_adoption_commit_approval_parser.add_argument("--approved-by", type=str, default="")
    phase_backend_created_output_adoption_commit_approval_parser.add_argument("--reason", type=str, default="")
    phase_backend_created_output_adoption_commit_approval_parser.set_defaults(handler=run_phase_backend_created_output_adoption_commit_approval)

    phase_backend_created_output_adoption_commit_approval_show_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-commit-approval-show",
        help="Show latest backend-created output adoption commit approval artifact (Phase 77R).",
    )
    ...
    # Phase 77T
    phase_backend_created_output_adoption_push_approval_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-push-approval",
        help="Push approval for backend-created output adoption bundle (Phase 77T).",
    )
    phase_backend_created_output_adoption_push_approval_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_push_approval_parser.add_argument("--save", action="store_true")
    phase_backend_created_output_adoption_push_approval_parser.add_argument("--approve", action="store_true")
    phase_backend_created_output_adoption_push_approval_parser.add_argument("--approved-by", type=str, default="")
    phase_backend_created_output_adoption_push_approval_parser.add_argument("--reason", type=str, default="")
    phase_backend_created_output_adoption_push_approval_parser.set_defaults(handler=run_phase_backend_created_output_adoption_push_approval)

    phase_backend_created_output_adoption_push_approval_show_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-push-approval-show",
        help="Show latest backend-created output adoption push approval artifact (Phase 77T).",
    )
    phase_backend_created_output_adoption_push_approval_show_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_push_approval_show_parser.set_defaults(handler=run_phase_backend_created_output_adoption_push_approval_show)

    # Phase 77U
    phase_backend_created_output_adoption_push_execution_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-push-execution",
        help="Push execution for backend-created output adoption bundle (Phase 77U).",
    )
    phase_backend_created_output_adoption_push_execution_parser.add_argument("--json", action="store_true")
    phase_backend_created_output_adoption_push_execution_parser.add_argument("--save", action="store_true")
    phase_backend_created_output_adoption_push_execution_parser.add_argument("--dry-run", action="store_true")
    phase_backend_created_output_adoption_push_execution_parser.add_argument("--execute", action="store_true")
    phase_backend_created_output_adoption_push_execution_parser.set_defaults(handler=run_phase_backend_created_output_adoption_push_execution)

    phase_backend_created_output_adoption_push_execution_show_parser = phase_subparsers.add_parser(
        "backend-created-output-adoption-push-execution-show",
        help="Show latest backend-created output adoption push execution artifact (Phase 77U).",
    )
    ...
    phase_runner_sim_review_parser = phase_subparsers.add_parser(
        "runner-sim-review",
        help="Review the latest runner simulation artifact for approval readiness.",
    )
    phase_runner_sim_review_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_runner_sim_review_parser.add_argument(
        "--save", action="store_true", help="Save review to .pcae/runner-simulation-reviews/."
    )
    phase_runner_sim_review_parser.set_defaults(handler=run_phase_runner_sim_review)

    phase_runner_simulate_parser = phase_subparsers.add_parser(
        "runner-simulate",
        help="Run a dry-run simulation trace combining readiness, plan, policy, and fixture entries.",
    )
    phase_runner_simulate_parser.add_argument(
        "--json", action="store_true", help="Print machine-readable JSON output."
    )
    phase_runner_simulate_parser.add_argument(
        "--save", action="store_true", help="Save simulation trace to .pcae/runner-simulations/."
    )
    phase_runner_simulate_parser.add_argument(
        "--count", type=int, default=3,
    ...
    lifecycle_run_gate_parser = lifecycle_boa_subparsers.add_parser(
        "run-gate",
        help="Dry-run gate evaluation (read-only, --dry-run required).",
    )
    lifecycle_run_gate_parser.add_argument("--gate", required=True, help="Gate ID to evaluate.")
