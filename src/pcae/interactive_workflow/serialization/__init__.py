...
``EvidenceItem``, ``Clarification``, and ``AuditEvent``, and in Phase
143N with sibling serializers for ``Preview``, ``ConfirmationRequest``,
and ``ConfirmationResponse``. Each artifact class's serializer is exposed
under its own name (``evidence_to_payload``/``evidence_from_payload``,
etc.) rather than a single overloaded ``to_payload``/``from_payload``
pair, so a caller's import makes explicit which artifact class it is
serializing -- no publication or CHGR serializer exists here, since this
package implements neither artifact.
"""
