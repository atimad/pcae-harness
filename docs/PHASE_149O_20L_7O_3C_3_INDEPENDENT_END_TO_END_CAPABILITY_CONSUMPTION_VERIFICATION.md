...
- **Fast Green, genuine A/B** (this phase's own diff touches zero `src/pcae` files — see `git diff --stat 9139a2bb..HEAD`):
  - **Baseline** (phase-entry HEAD `9139a2bb`, disposable `git worktree`, clean): `338 failed, 8689 passed, 5 skipped, 27721 deselected, 9 errors (448.52s)` — matches 3C.2's own previously-recorded baseline count exactly.
  - **With this phase's diff** (tests/docs/task-lifecycle files only, run twice for reproducibility, both runs identical): `339 failed, 8688 passed, 5 skipped, 27743 deselected, 9 errors (~527s)`.
  - **Nodeid-level diff** (`comm` between the two failing-node sets): exactly one newly-failing node not in baseline, `test_phase_149o_20l_7n_1_dell_redeployment_proposition_independent_verification.py::TestCandidateCurrentness::test_head_equals_origin_main` — expected and transient (this phase's commits are ahead of `origin/main` at the point the run was taken; the identical pattern 3C.2's own report documented, resolved by this phase's own push). One pre-existing-category node swapped within the same file/class (`test_shell_gate.py::TestAuditPersistence`): `test_verify_detects_tampered_record` passed instead of failing while `test_audit_verify_cli` failed instead of passing — both members of an already pre-existing, unrelated (shell-gate-audit) failing category; the swap is consistent with test-order/shared-state sensitivity within that class (collection order shifts when any new test file, e.g. this phase's own, is added anywhere in the suite), not a functional regression — net failing-count contribution from this pair is zero either way, and neither test touches any file this phase's diff modifies.
  - **Deselected clean run:** deselecting the 347 baseline nodeids plus the two explained deltas above (349 total): **0 failed, 8687 passed, 5 skipped, 28092 deselected, 0 errors (480.92s).** Zero attributable regressions.
