...
**Governed tool result** (`pcae phase fast-green-attribution`,
`baseline_method: parent_of_oldest_phase_attributed_commit`, baseline
`ac108efd` vs. candidate `9854a8ff`, isolated-worktree baseline-vs-candidate
comparison — the authoritative attribution mechanism, distinct from and
run in addition to the manual `git stash` sweep above): **1 attributable
failure**, not zero:
...
Read directly (unmodified): this test loads
`docs/evidence/helper-installation-identity/baseline.json`, asserts it
equals a hardcoded baseline commit sha (`79ea7e1644535d011da6ca3869b5557b44c50737`,
from the earlier N16-5-F-5-TB-HELPER-INSTALLATION-IDENTITY-CONTRACT-REPAIR
phase), and then re-hashes **every** file under `src/pcae/**` against the
sha256 values recorded at that commit. It is a full-tree source freeze, by
construction: it will fail after *any* subsequent legitimate change to
*any* file under `src/pcae/`, forever, until its recorded baseline is
regenerated by a dedicated phase for that purpose. This phase's three
authorized edits under `src/pcae/core/` necessarily trip it. This is not a
functional or security regression — no behavior this test's own hashes
protect (installation identity) was touched; the three edited files
(`hpac_pawa_helper_protocol.py`, `hpac_pawa_helper_store_adapter.py`,
`hpac_pawa_helper_operations.py`) are not installation-identity files, they
simply happen to live under the same `src/pcae/**` glob the freeze test
covers unconditionally.

Per phase instructions ("do not edit [historical tests] merely to make
green"), this test and `baseline.json` are left byte-unchanged: fixing it
would require either editing the test's hardcoded baseline-commit
assertion (out of this phase's 3-file scope) or regenerating
`baseline.json` in a way that still fails the hardcoded commit-sha check —
neither is achievable within this phase's narrow authorization without
itself becoming an out-of-scope edit. This is flagged here as an
explained, non-suppressed, single attributable item, not represented as
"zero regressions." A follow-up housekeeping phase should refresh that
baseline under its own governed authorization.

**Net result: zero *unexplained* regressions; one explained, non-security,
non-functional attributable item** (the full-tree baseline-freeze test
above), disclosed rather than silently reclassified. Every other failing
...
repair (documented above per test name).
...
| Fast Green | 1 attributable failure — explained, non-security full-tree baseline-freeze pin, not silently zeroed (§5) |
...
tests, and disclosed one explained, non-security, non-functional
attributable Fast Green item (§5) rather than claiming a false zero — but
per phase authorization, does not and cannot certify itself as
