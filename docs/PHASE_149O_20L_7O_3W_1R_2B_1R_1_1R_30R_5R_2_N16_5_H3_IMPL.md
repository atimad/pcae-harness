...
| H-3 test suite — part 2 (deterministic end-to-end chain reachability through the coordinator: open_challenge → record_assertion → record_verified_proof → reach_gate5_assurance → PRODUCTION `AuthenticatedHumanPrincipal` → real Gate 5; deterministic auth / deterministic presentation non-elevation; mixed non-real negatives; PB/policy DENY dominance; challenge/proof/counter/presentation/principal forgery negatives) | — | pending |
