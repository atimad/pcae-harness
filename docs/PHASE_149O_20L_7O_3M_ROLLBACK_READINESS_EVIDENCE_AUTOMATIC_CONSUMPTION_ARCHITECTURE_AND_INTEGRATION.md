...
### 26A. Fast Green push-state attribution stabilization

Three `pcae phase fast-green-attribution` invocations were run this phase, not one. This section records all three and separates functional attribution from push-state verification explicitly, per the two-class model below.

**Why three runs.** The tool's `expected_phase_artifacts` prediction is driven by a caller-supplied `--pushed-status` argument that must agree with the finalization gate's own *live* git-derived push-state classification at validation time. As this phase's own commits/pushes progressed through the governed lifecycle (not-yet-pushed → pushed), that live classification changed between attempts, and each mismatch (`local_only` vs. `not_pushed` vs. `pushed`) required a fresh invocation with the corrected argument to pass the metadata trust gate's structured-evidence validator. This is a caller-argument/gate-state churn problem, not evidence of a functional regression.

| Run | Candidate commit | `--pushed-status` | Result | Purpose |
|---|---|---|---|---|
| 1 (clean) | `e632a2df` (the exact product/integration commit) | *(default)* `local_only` | PASS, 0 attributable | First valid functional evidence (a first attempt was corrupted by two concurrent invocations racing on one output file — author error, discarded) |
| 2 | `cc2fc0d5` (metadata-only commit) | `not_pushed` | PASS, 0 attributable | Re-aligned `expected_phase_artifacts` with the finalization gate's live `not_pushed` classification |
| 3 (final) | `4ec66eee` (metadata/docs-only commit, post-push) | `pushed` | FAIL, 1 attributable (see below) | Re-aligned with the gate's live `pushed` classification after this phase's governed push |

**FUNCTIONAL ATTRIBUTION (authoritative): Run 1.** Baseline `7b193145` (phase-entry commit), candidate `e632a2df` (the commit containing every production/test change this phase makes). `git diff --stat e632a2df..4ec66eee -- src/pcae/ tests/` is independently confirmed **empty** — every commit after `e632a2df` (metadata, Fast Green artifacts, phase document, task lifecycle) touches only `.pcae/`, `docs/`, `tasks/`, `PROJECT_STATUS.md`, `CHANGELOG.md`. Run 1's result therefore remains fully valid for the final pushed state: **0 attributable functional regressions**.

**Run 3's one candidate-only node**, `tests/test_shell_gate.py::TestAuditPersistence::test_verify_detects_tampered_record`, was investigated directly rather than dismissed or re-run at full scale: it passed on three consecutive isolated re-invocations; it imports only `pcae.core.shell_gate` (zero dependency relationship to this phase's two touched files); a *different* test in the same file (`test_audit_verify_cli`) independently failed via a hardcoded 15-second subprocess `TimeoutExpired` when the file was run serially immediately afterward, confirming this file's suite is timing/load-sensitive in this environment under the sustained load of three consecutive full Fast Green passes, not deterministically broken; and zero production or test source changed between Run 1's candidate and Run 3's candidate. **Classification: environment-induced flakiness (equivalent to `excluded_environment_failures`), not an attributable regression.** No fourth full run was performed to reclassify it formally via `--rerun-node`, per the governing brief's explicit instruction not to repeat full suite runs solely to chase mutable push-state/argument churn once functional attribution and the flake's non-relatedness were independently established.

**FINAL PUSH-STATE VERIFICATION (separate from functional attribution).** Performed once, after the final governed push (`cb0d133f`), with zero additional Fast Green invocation: `git status --short` empty; `git rev-parse HEAD` == `git rev-parse origin/main` == `cb0d133fdb139897c540ead4db2bb174219c5968`; `git rev-list --count origin/main..HEAD` == `0`; `pcae push check` → `Mode: nothing_to_push`; `pcae status coherence` → `coherent`; `pcae runtime inspect` → `Observed`/`observe`/`unavailable`, unchanged.

**Attributable functional regressions across the full stabilization: 0.**

**Deferred infrastructure finding (non-blocking, not repaired this phase):** FAST GREEN PUSH-STATE ATTRIBUTION is STATE-SENSITIVE / SELF-REFERENTIAL — the caller-supplied `--pushed-status` argument can go stale mid-lifecycle as a governed phase's own commits/pushes change the live push-state classification, forcing redundant re-invocations purely to keep the structured-evidence schema's `expected_phase_artifacts` prediction aligned with gate-computed reality. No product-correctness impact was demonstrated. Future hardening (out of this phase's scope): derive pushed status internally from the tested repository/worktree at run time rather than accepting a caller-supplied prediction, or separate the self-referential push-state sentinel test from source-behavior Fast Green attribution entirely.
