...
`linux-probe.json` contains actual peer credentials, all three ID cases, the metadata-read exception and the direct foundation exception. `linux_probe.py` is the exact probe. Same-account explicit identity: DENY. None: incorrectly ACCEPT (baseline defect remains). Distinct explicit identity: peer primitive PASS only, **not verified end-to-end admission**. Forged-context/direct-entrypoint/wrong-agent/copy/restart/all-five repaired-denial and valid read/write dispatch criteria: **NOT IMPLEMENTED / NOT VERIFIED**, blocked by B2/B3; never reported as passing. No protected write succeeded through Model E. Disposable setup removed; host canonical root remains absent; cleanup recorded in linux-cleanup.txt. Live protected-host writes = 0.

Fast Green: governed attribution against first phase commit's parent `fabbfac07276b7e9b44b25295943d24e77a27229`; final evidence must be regenerated at final pushed HEAD. Pending lifecycle evidence is recorded separately as it becomes available; no unit-only success claim.
